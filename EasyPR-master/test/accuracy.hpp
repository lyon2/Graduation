...
namespace easypr {
  ...
  namespace demo {
    ...
    int accuracyTest(const char* test_path, Result& result, bool useParams = false) {
      ...
      cout<<"accuracyTest12345678"<<endl;
      map<string, vector<CPlate>> xmlMap;
      string path(test_path);
      path = path + "/GroundTruth.xml";
      getGroundTruth(xmlMap, path.c_str());
      map<string, vector<CPlate>>::iterator it2; 
      
      //for(it2=xmlMap.begin();it2!=xmlMap.end();it2++)
        //cout<<(*it2).first<<endl;
...
#pragma omp critical
        {
          ...
          //cout<<plateLicense<<(*it).first<<endl;
          //cout<<"here"<<plateLicense<<(it==xmlMap.end())<<endl;   
          if (it != xmlMap.end()) {
